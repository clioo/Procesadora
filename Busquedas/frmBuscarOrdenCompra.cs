...
namespace Procesadora
{
    public partial class frmBuscarOrdenCompra : Form
    {
        ...
        {
            dgvResultados.DataSource = conexion.Select("*", "v_ordenCompra", "status = 'A'");
        ...
        private void txtValor_TextChanged(object sender, EventArgs e)
        {
            if (!(txtValor.Text == ""))
            {
                dgvResultados.DataSource = conexion.Select("*", "v_ordenCompra", cbBuscarPor.SelectedValue.ToString() + " like " + comillas(txtValor.Text + "%"));
            }
        }
